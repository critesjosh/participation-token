...
exports.shouldBehaveLikeStandardToken = function () {
...
/////// WHEN A TOKEN SHOULD NOT ALLOW STANDARDTOKEN FUNCTION EXECUTIONS

exports.shouldNOTBehaveLikeStandardToken = function () {
  contract('StandardToken FAIL', function ([_, owner, recipient, anotherAccount]) {

    const amount = 1;
    const to = recipient;
    const spender = recipient;

    it('owner should have @initialBalance tokens', async function () {
      const balance = await this.token.balanceOf(owner);
      assert.equal(balance, 100);
    });

    it('should NOT allow call #transfer', async function(){
      await assertRevert(this.token.transfer(to, amount, { from: owner }));
    });

    it('should NOT allow call #transferFrom', async function(){
      await assertRevert(this.token.transferFrom(owner, to, amount, { from: owner }));
    });

    it('should NOT allow call #approve', async function(){
      await assertRevert(this.token.approve(spender, amount, { from: owner }));
    });

    it('should NOT allow call #increaseApproval', async function(){
      await assertRevert(this.token.increaseApproval(spender, amount, { from: owner }));
    });

    it('should NOT allow call #decreaseApproval', async function(){
